...
from janest_model import CustomDataset, train_lgb, ResNetModel, SmoothBCEwLogits, utility_score_bincount, TransformerModel, log_evaluation
...
def main():
    ...
    VER = (VER + '_' + EXT)
    model = ResNetModel(input_size = X.shape[-1], output_size = y.shape[-1],
#                         noise=0.2
                       ).to(DEVICE)
